...
const verifyUserAssets = async (req, res) => {
  try {
   ...
   let  storyContent = stories.content;
    let requiredAssetIdsCommaSeparated = [];

    for (i = 0; i < requiredTemplateIds.length; i++) {
      await axios
        .get(
          `https://wax.api.atomicassets.io/atomicassets/v1/assets?template_id=${requiredTemplateIds[i]}&page=1&limit=1000&order=desc&sort=asset_id`
        )
        .then((response) => {
          const { data } = response.data;
